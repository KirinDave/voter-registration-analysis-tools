<?php
/*
Looks at address_plus_set.txt - containing all dobs and all lastvoted dates, among other things
...
fwrite( $outputfile, "ID	Name	DOB	Age	Gender	Phone	Address	County	Party	Status	Changed	Last Voted	Grep Code\n" );
...
foreach( $addressesetc as $key=>$addressetc )
{
	...
	//initialize diffsecs to 1000 years
	$diffseconds = 1000*365*24*60*60;
	if( $lastvoteddate && $dob )
	{
		$lastvotedjulian = strtotime( $lastvoteddate );
		$dobjulian = strtotime( $dob );
		$diffseconds = $lastvotedjulian - $dobjulian;
		$agewhenlastvoted = round( $diffseconds / (365*24*60*60 ), 1 );
	}
	
	// check if difference between "last voted" date and age is less than 18 years
	if( $diffseconds < 18*365*24*60*60 && $lastvoteddate != '01/01/1800' )
	{
		$count++;
		echo $justname."\t".$dob."\t".$agewhenlastvoted."\t".$gender."\n";
		fwrite( $outputfile, trim( $fullid )."\t".$justname."\t".$dob."\t".$agewhenlastvoted."\t".$gender."\t".$phone."\t".$address."\t".$countyname."\t".$party."\t".$status."\t".$recordchangeddate."\t".$lastvoteddate."\t".$paddedcountycode.'xxxxxxx'."\n" );
	}
}

echo 'Underage voters: '.$count."\n\n";
