...
fwrite( $nameoutputfile, "Previous ID	ID	Previous Name	Name	Previous DOB	DOB	Previous Gender	Gender	Previous Phone	Phone	Previous Address	Previous County	Address	County	Previous Party	Party	Previous Status	Status	Previous Changed	Changed	Previous Last Voted	Last Voted	Grep Code\n" );

$doboutputfilename = 'changedidentities/changeddobs_'.$argv[1].'_'.$argv[2].'.txt';	
$doboutputfile = fopen( $doboutputfilename, 'w' );
fwrite( $doboutputfile, "Previous ID	ID	Previous Name	Name	Previous DOB	DOB	Previous Gender	Gender	Previous Phone	Phone	Previous Address	Previous County	Address	County	Previous Party	Party	Previous Status	Status	Previous Changed	Changed	Previous Last Voted	Last Voted	Grep Code\n" );

$genderoutputfilename = 'changedidentities/changedgenders_'.$argv[1].'_'.$argv[2].'.txt';
$genderoutputfile = fopen( $genderoutputfilename, 'w' );
fwrite( $genderoutputfile, "Previous ID	ID	Previous Name	Name	Previous DOB	DOB	Previous Gender	Gender	Previous Phone	Phone	Previous Address	Previous County	Address	County	Previous Party	Party	Previous Status	Status	Previous Changed	Changed	Previous Last Voted	Last Voted	Grep Code\n" );

$anythingoutputfilename = 'changedidentities/changedanything_'.$argv[1].'_'.$argv[2].'.txt';
$anythingoutputfile = fopen( $anythingoutputfilename, 'w' );
fwrite( $anythingoutputfile, "Previous ID	ID	Previous Name	Name	Previous DOB	DOB	Previous Gender	Gender	Previous Phone	Phone	Previous Address	Previous County	Address	County	Previous Party	Party	Previous Status	Status	Previous Changed	Changed	Previous Last Voted	Last Voted	Grep Code\n" );

$partyoutputfilename = 'changedidentities/changedparties_'.$argv[1].'_'.$argv[2].'.txt';
$partyoutputfile = fopen( $partyoutputfilename, 'w' );
fwrite( $partyoutputfile, "Previous ID	ID	Previous Name	Name	Previous DOB	DOB	Previous Gender	Gender	Previous Phone	Phone	Previous Address	Previous County	Address	County	Previous Party	Party	Previous Status	Status	Previous Changed	Changed	Previous Last Voted	Last Voted	Grep Code\n" );
...
fwrite( $lastvotediscrepancyoutputfile, "Previous ID	ID	Previous Name	Name	Previous DOB	DOB	Previous Gender	Gender	Previous Phone	Phone	Previous Address	Previous County	Address	County	Previous Party	Party	Previous Status	Status	Previous Changed	Changed	Previous Last Voted	Last Voted	Grep Code\n" );
...
foreach( $voterids1 as $key1=>$voterid1 )
{
	...
	if( $key2 )
	{
		...
		foreach( $addressetc as $key=>$addressetc)
		{
			...
			$addresses[ $key ] = str_replace( $addressparts[0], '', $address );
			$phones[ $key ] = $parts[1];
			$parties[ $key ] = $parts[2];
		...
		if( ( ( $argv[1] == '2016-11-28' || $argv[1] == '2017-02-27' ) && ( $lastvoteddates[ 2 ] != '03/21/2017' && $lastvoteddates[ 2 ] != '05/16/2017' && $lastvoteddates[ 1 ] != $lastvoteddates[ 2 ] ) ) || ( $argv[2] == '2016-11-28' && ( $lastvoteddates[ 2 ] != '04/26/2016' && $lastvoteddates[ 2 ] != '07/12/2016' && $lastvoteddates[ 2 ] != '11/08/2016' && $lastvoteddates[ 1 ] != $lastvoteddates[ 2 ] ) ) )
		{
			...
			fwrite( $lastvotediscrepancyoutputfile, trim( $fullids[1] )."\t".trim( $fullids[2] )."\t".$justnames[1]."\t".$justnames[2]."\t".$dobs[1]."\t".$dobs[2]."\t".$genders[1]."\t".$genders[2]."\t".$phones[1]."\t".$phones[2]."\t".$addresses[1]."\t".$countynames[1]."\t".$addresses[2]."\t".$countynames[2]."\t".$parties[1]."\t".$parties[2]."\t".$statuses[1]."\t".$statuses[2]."\t".$recordchangeddates[1]."\t".$recordchangeddates[2]."\t".$lastvoteddates[1]."\t".$lastvoteddates[2]."\t".$paddedcountycode.'xxxxxxx'."\n" );
		}
		if( $names[1] != $names[2]  )
		{
			fwrite( $anythingoutputfile, trim( $fullids[1] )."\t".trim( $fullids[2] )."\t".$justnames[1]."\t".$justnames[2]."\t".$dobs[1]."\t".$dobs[2]."\t".$genders[1]."\t".$genders[2]."\t".$phones[1]."\t".$phones[2]."\t".$addresses[1]."\t".$countynames[1]."\t".$addresses[2]."\t".$countynames[2]."\t".$parties[1]."\t".$parties[2]."\t".$statuses[1]."\t".$statuses[2]."\t".$recordchangeddates[1]."\t".$recordchangeddates[2]."\t".$lastvoteddates[1]."\t".$lastvoteddates[2]."\t".$paddedcountycode.'xxxxxxx'."\n" );
			...
			if( $justnames[1] != $justnames[2] )
			{
				...
				//write
				echo 'NAME CHANGE '.trim( $fullids[2] )."\t".$justnames[2]."\t".$justnames[1]."\t".$dobs[2]."\t".$genders[2]."\n";
				
				fwrite( $nameoutputfile, trim( $fullids[1] )."\t".trim( $fullids[2] )."\t".$justnames[1]."\t".$justnames[2]."\t".$dobs[1]."\t".$dobs[2]."\t".$genders[1]."\t".$genders[2]."\t".$phones[1]."\t".$phones[2]."\t".$addresses[1]."\t".$countynames[1]."\t".$addresses[2]."\t".$countynames[2]."\t".$parties[1]."\t".$parties[2]."\t".$statuses[1]."\t".$statuses[2]."\t".$recordchangeddates[1]."\t".$recordchangeddates[2]."\t".$lastvoteddates[1]."\t".$lastvoteddates[2]."\t".$paddedcountycode.'xxxxxxx'."\n" );
			}
			if( $genders[1] != $genders[2] )
			{
				...
				//write
				echo 'GENDER CHANGE '.trim( $fullids[2] )."\t".$justnames[2]."\t".$dobs[2]."\t".$genders[2]."\t".$genders[1]."\n";
				
				fwrite( $genderoutputfile, trim( $fullids[1] )."\t".trim( $fullids[2] )."\t".$justnames[1]."\t".$justnames[2]."\t".$dobs[1]."\t".$dobs[2]."\t".$genders[1]."\t".$genders[2]."\t".$phones[1]."\t".$phones[2]."\t".$addresses[1]."\t".$countynames[1]."\t".$addresses[2]."\t".$countynames[2]."\t".$parties[1]."\t".$parties[2]."\t".$statuses[1]."\t".$statuses[2]."\t".$recordchangeddates[1]."\t".$recordchangeddates[2]."\t".$lastvoteddates[1]."\t".$lastvoteddates[2]."\t".$paddedcountycode.'xxxxxxx'."\n" );
			}
			if( $dobs[1] != $dobs[2] )
			{
				// look at changed identity
				...
				//write
				echo 'DOB CHANGE '.trim( $fullids[2] )."\t".$justnames[2]."\t".$dobs[1]."\t".$dobs[2]."\t".$genders[2]."\n";
				fwrite( $doboutputfile, trim( $fullids[1] )."\t".trim( $fullids[2] )."\t".$justnames[1]."\t".$justnames[2]."\t".$dobs[1]."\t".$dobs[2]."\t".$genders[1]."\t".$genders[2]."\t".$phones[1]."\t".$phones[2]."\t".$addresses[1]."\t".$countynames[1]."\t".$addresses[2]."\t".$countynames[2]."\t".$parties[1]."\t".$parties[2]."\t".$statuses[1]."\t".$statuses[2]."\t".$recordchangeddates[1]."\t".$recordchangeddates[2]."\t".$lastvoteddates[1]."\t".$lastvoteddates[2]."\t".$paddedcountycode.'xxxxxxx'."\n" );
			}
			if( $parties[1] != $parties[2] )
			{
				fwrite( $partyoutputfile, trim( $fullids[1] )."\t".trim( $fullids[2] )."\t".$justnames[1]."\t".$justnames[2]."\t".$dobs[1]."\t".$dobs[2]."\t".$genders[1]."\t".$genders[2]."\t".$phones[1]."\t".$phones[2]."\t".$addresses[1]."\t".$countynames[1]."\t".$addresses[2]."\t".$countynames[2]."\t".$parties[1]."\t".$parties[2]."\t".$statuses[1]."\t".$statuses[2]."\t".$recordchangeddates[1]."\t".$recordchangeddates[2]."\t".$lastvoteddates[1]."\t".$lastvoteddates[2]."\t".$paddedcountycode.'xxxxxxx'."\n" );
			}
		}
	}
	//do it again if there's a dupe
	if( $nextkey )
	{
		$names[1] = trim( $names1[ $key1 ] );
		$names[2] = trim( $names2[ $nextkey] );
		$fullids[1] = trim( $fullids1[ $key1 ] );
		$fullids[2] = trim( $fullids2[ $nextkey] );
		$addressesetc[1] = trim( $addressesetc1[ $key1 ] );
		$addressesetc[2] = trim( $addressesetc2[ $nextkey] );
		// get county names from full ids
		foreach( $fullids as $key=>$fullid )
		{
			$parts = explode( '-', $fullid );
